 ...
 * @time 2016-9-19
 */
public class MDPlayerDetailsActivity extends AppCompatActivity implements View.OnClickListener, CustomMediaPlayer.OnNetChangeListener {
...
//        url = "http://baobab.wandoujia.com/api/v1/playUrl?vid=2614&editionType=normal".trim();
    ...
    private void initPlayer() {
        player = (CustomMediaPlayer) findViewById(R.id.view_super_player);
        if (isLive) {
            player.setLive(true);//设置该地址是直播的地址
        }
        player.setNetChangeListener(true)//设置监听手机网络的变化
                .setOnNetChangeListener(this)//实现网络变化的回调
                .onPrepared(new CustomMediaPlayer.OnPreparedListener() {
                    @Override
        ...
        }).onInfo(new CustomMediaPlayer.OnInfoListener() {
            @Override
        ...
        }).onError(new CustomMediaPlayer.OnErrorListener() {
            @Override
            public void onError(int what, int extra) {
                ...
                .play(url);//开始播放视频
        player.setScaleType(CustomMediaPlayer.SCALETYPE_FITXY);
    }
